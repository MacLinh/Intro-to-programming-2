...
public class Statistics {
    ...
    /**
     * the minimum and maximum value of the current data set
     */
    ...
    public double standardDeviation(){
        ...
        if(this.avg == -1)// if average was not yet calculated
            avg = average();
        else // else use the average to save time
            avg = this.avg;
        double sum = 0;
        for (int i = 0; i < index; i++){
    ...
    public String toString(){
        if (!updated)// this will save time so calculations aren't performed every time the object is printed...
